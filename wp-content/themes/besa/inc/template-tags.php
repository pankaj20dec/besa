 ...
 */
 ...
 if (!function_exists('twentysixteen_entry_meta2')):
    function twentysixteen_entry_meta2()
    {
        if ('post' === get_post_type()) {
           
			$categories_list = get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'smoke'));
            if ($categories_list && twentysixteen_categorized_blog()) {
                printf('<span class="category-name">%1$s</span>', $categories_list);
            }
            $author_avatar_size = apply_filters('smoke_author_avatar_size', 49);
            printf('<span class="author-name"><a class="url fn n" href="%1$s">%2$s</a></span>', esc_url(get_author_posts_url(get_the_author_meta('ID'))), ucwords(get_the_author()));
        }
    }
endif;
